...
El primer bono se procesa con el cursor vacío — no hay tasas previas conocidas. Esto significa que $T_{\text{anterior}} = 0$ y $R_{\text{anterior}} = 0$, por lo que **todos** sus flujos caen en el tramo nuevo y se resuelven con la misma lógica iterativa general (ver Sub-fase 4.2). No existe un caso especial de inicialización separado.

En la práctica, si el primer bono tiene $\tau < 1$ año y no tiene cupones intermedios, la ecuación se reduce a:

$$B_1 = \frac{P_{\text{sucio}}}{1 + \text{couponRate}/100} \qquad \Rightarrow \qquad r_{\text{spot},1} = B_1^{-1/\tau_1} - 1$$

Pero esto es simplemente el caso particular del despeje algebraico cuando la Categoría B está vacía y $T_{\text{anterior}} = 0$ — no una lógica aparte.

#### Sub-fase 4.2 — Iteración Recursiva

Para cada bono $i$ (todos, incluido el primero con cursor vacío):

1. Obtener los flujos de caja del bono $i$ desde M3.

2. **Clasificar los flujos en tres categorías estrictas:**

   | Categoría | Condición | Tratamiento |
   |---|---|---|
   | **A — conocidos** | $t \leq T_{\text{anterior}}$ | Descuento con cursor de tasas ya calculadas |
   | **B — tramo nuevo** | $T_{\text{anterior}} < t < \tau_i$ | Dependen obligatoriamente de $f$ (incógnita) |
   | **C — flujo final** | $t = \tau_i$ | Depende de $f$ (incógnita) |

   > **Error crítico a evitar:** clasificar los flujos en solo dos grupos (anteriores vs final) es incorrecto. Los flujos de Categoría B caen en el tramo nuevo y su VP depende de $f$ — no del cursor. Descontarlos con tasas del cursor contamina el VP residual y produce tasas forward erróneas.

3. Calcular el **VP residual puro** restando del precio sucio **únicamente** los flujos de Categoría A:
$$VP_{\text{residual puro}} = P_{\text{sucio},i} - \sum_{\text{Cat. A}} \frac{CF_k}{FD_{\text{cursor}}(t_k)}$$

4. **Evaluar si Categoría B está vacía:**

   - **B vacío → despeje algebraico exacto:**
   $$VP_{\text{residual puro}} = \frac{CF_{\text{final}}}{(1 + R_{\text{anterior}})^{T_{\text{anterior}}} \cdot (1 + f)^{\Delta T}}$$
   $$f = \left[\frac{CF_{\text{final}}}{VP_{\text{residual puro}} \cdot (1 + R_{\text{anterior}})^{T_{\text{anterior}}}}\right]^{1/\Delta T} - 1$$

   - **B no vacío → solver numérico obligatorio (`brentq`):**
   La incógnita $f$ aparece en múltiples denominadores con exponentes distintos, formando un polinomio sin solución algebraica. Se resuelve numéricamente igualando:
   $$VP_{\text{residual puro}} = \underbrace{\sum_{\text{Cat. B}} \frac{CF_b}{(1+R_{\text{ant}})^{T_{\text{ant}}} \cdot (1+f)^{t_b - T_{\text{ant}}}}}_{\text{flujos intermedios}} + \underbrace{\frac{CF_{\text{final}}}{(1+R_{\text{ant}})^{T_{\text{ant}}} \cdot (1+f)^{\Delta T}}}_{\text{flujo final}}$$

5. Calcular $R_{\text{spot},i}$ usando la equivalencia geométrica exacta (Sección 2.5).

6. Almacenar el par $(\tau_i, R_{\text{spot},i})$ y actualizar el cursor.

#### Sub-fase 4.3 — Manejo de Casos Degenerados

El algoritmo debe ser robusto ante situaciones atípicas:

- **VP residual puro negativo:** puede ocurrir por precios de mercado extremos o inconsistencias de datos. El algoritmo registra una advertencia y omite ese bono.
- **$f$ fuera de rango** (< −0.05 o > 0.50): señal de inconsistencia en los datos del día. Se registra y el bono se omite.
- **$\Delta T \approx 0$:** el tramo nuevo es demasiado pequeño para ser numéricamente estable (dos bonos con vencimientos casi idénticos que pasaron el filtro de 30 días). El bono se omite con advertencia.
- **Solver numérico no converge:** cuando Categoría B no está vacía y `brentq` no encuentra raíz en el intervalo $[-0.15, 1.0]$. El bono se omite con advertencia.
- **Menos de 2 puntos spot en el día:** el día se marca como `INSUFICIENTE` y se excluye del panel histórico.
